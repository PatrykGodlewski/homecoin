...
if command -v docker >/dev/null 2>&1 && docker compose ps postgres 2>/dev/null | grep -q "running\|Up"; then
  ...
  docker compose exec -T postgres psql -U homecoin -d homecoin < "$ROOT/scripts/dev/seed.sql"
elif command -v psql >/dev/null 2>&1; then
  echo "==> Seeding via psql"
  psql "$DATABASE_URL" -f "$ROOT/scripts/dev/seed.sql"
  ...
  echo "  make seed"
  echo "  ./scripts/dev/seed_db.sh"
  echo "  psql \"\$DATABASE_URL\" -f scripts/dev/seed.sql"
  exit 1
fi
